...
$s = htmlspecialchars($_SESSION['s']);
$jeux = $Connect->query('SELECT * FROM jeux WHERE jeu LIKE "%' . $s . '%" OR genre LIKE "%' . $s . '%" OR description LIKE "%' . $s . '%" ORDER BY jeu ASC');

// on sélectionne tous les jeux de la table jeux ayant dans leur nom, dans leur genre ou dans leur descritpion la chaine de caractères entrée par l'utilisateur
...
<head>
  <meta charset="UTF-8" />
  <title>Résultat(s) - Le bistro du JV</title>
...
<body>
  ...
  <main>
    <h1>
      <i class="fas fa-search h1-icon"></i>
      <span>résultat(s) pour "<?= $s ?>"</span>
      <hr color="black">
    </h1>
